...
void init_thr_alarm(uint max_alarms)
{
  ...
  printf("%s[%d] [tid:%lu]: thr_client_alarm = %d...", __FILE__, __LINE__, pthread_self(), thr_client_alarm);
...
void thr_alarm_kill(my_thread_id thread_id)
{
  ...
  printf("%s[%d] [tid:%lu]: Seting alarm expire_time = 0 for the thread {id = %ld}...", __FILE__, __LINE__, pthread_self(), thread_id);
