...
size_t vio_read(Vio * vio, uchar* buf, size_t size)
{
  ...
  printf("%s[%d] [tid:%lu]: Reading data from client(%s), buffer size = %u...\n", __FILE__, __LINE__, pthread_self(), vio->_debug_ip_port, size);
...
int vio_close(Vio * vio)
{
 ...
 if (vio->type != VIO_CLOSED)
  {
	 printf("%s[%d] [tid:%lu]: Closing the network i/o from client(%s)...", __FILE__, __LINE__, pthread_self(), vio->_debug_ip_port);
