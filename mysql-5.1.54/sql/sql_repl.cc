		       ...
		       ushort flags)
{
  ...
  sql_print_information("%s[%d] [tid:%lu]: Try to send binlogs to the client(%s) from {log = %s, pos = %d, flags = %d}...", __FILE__, __LINE__, pthread_self(), thd->net.vio->_debug_ip_port, log_ident, pos, flags);
  ...
  sql_print_information("%s[%d] [tid:%lu]: binlog_open = %d, server_id_supplied = %d.", __FILE__, __LINE__, pthread_self(), mysql_bin_log.is_open(), server_id_supplied);
  ...
  if (!mysql_bin_log.is_open())
  {
    ...
    sql_print_information("%s[%d] [tid:%lu]: %s!", __FILE__, __LINE__, pthread_self(), errmsg);
  ...
  if ((file=open_binlog(&log, log_file_name, &errmsg)) < 0)
  {
    ...
    sql_print_information("%s[%d] [tid:%lu]: open binlog %s failed!", __FILE__, __LINE__, pthread_self(), log_file_name);
...
impossible position";
  ...
  */
  ...
  sql_print_information("%s[%d] [tid:%lu]: Sending binlogs to the client(%s)...", __FILE__, __LINE__, pthread_self(), thd->net.vio->_debug_ip_port);
  ...
  while (!net->error && net->vio != 0 && !thd->killed)
  {
	...
	goto err;
      ...
      sql_print_information("%s[%d] [tid:%lu]: Sending a binlog packet[%d] to the client(%s)...", __FILE__, __LINE__, pthread_self(), packet->length(), thd->net.vio->_debug_ip_port);
	...
	*/
	...
	case LOG_READ_EOF:
	  ...
	  if (!thd->killed)
	  {
		...
		sql_print_information("%s[%d] [tid:%lu]: Waiting for Sending newest binlogs to the client(%s)...", __FILE__, __LINE__, pthread_self(), thd->net.vio->_debug_ip_port);
...
int start_slave(THD* thd , Master_info* mi,  bool net_report)
{
  ...
  sql_print_information("%s[%d] [tid:%lu]: Start to start slave{server_id_supplied = %d}...", __FILE__, __LINE__, pthread_self(), server_id_supplied);
...
bool change_master(THD* thd, Master_info* mi)
{
  ...
  sql_print_information("%s[%d] [tid:%lu]: Start to CHANGE MASTER", __FILE__, __LINE__, pthread_self());
