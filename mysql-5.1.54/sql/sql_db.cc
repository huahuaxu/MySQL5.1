                                 ...
                                 CHARSET_INFO *new_db_charset)
{
  ...
  if (new_db_name == NULL)
  {
    /*
      ...
      sets the new one.
    */
	sql_print_information("%s[%d] [tid:%lu]: Setting the current DB[NULl] for client(%s)...", __FILE__, __LINE__, pthread_self(), thd->net.vio->_debug_ip_port);
    thd->set_db(NULL, 0);
  }
  else if (new_db_name == &INFORMATION_SCHEMA_NAME)
  {
    ...
    */
	sql_print_information("%s[%d] [tid:%lu]: Setting the current DB[%s] for client(%s)...", __FILE__, __LINE__, pthread_self(), INFORMATION_SCHEMA_NAME.str, thd->net.vio->_debug_ip_port);
  ...
  else
  {
    ...
    */
    ...
    sql_print_information("%s[%d] [tid:%lu]: Setting the current DB[%s] for client(%s)...", __FILE__, __LINE__, pthread_self(), new_db_name->str, thd->net.vio->_debug_ip_port);
...
bool mysql_change_db(THD *thd, const LEX_STRING *new_db_name, bool force_switch)
{
  ...
  */
  ...
  sql_print_information("%s[%d] [tid:%lu]: Checking ACL of user(%s:%s) from client(%s)...", __FILE__, __LINE__, pthread_self(),
		  sctx->priv_user, new_db_name->str, thd->net.vio->_debug_ip_port);
