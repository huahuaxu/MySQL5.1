...
bool table_cache_init(void)
{
  ...
  printf("%s[%d] [tid: %lu]: Initializing table chache...\n", __FILE__, __LINE__, pthread_self());
...
bool table_def_init(void)
{
  ...
  printf("%s[%d] [tid: %lu]: Initializing table define cache...\n", __FILE__, __LINE__, pthread_self());
...
int open_tables(THD *thd, TABLE_LIST **start, uint *counter, uint flags)
{
  ...
  */
  for (tables= *start; tables ;tables= tables->next_global)
  {
    ...
    sql_print_information("%s[%d] [tid:%lu]: Try to open table '%s'.'%s'...", __FILE__, __LINE__, pthread_self(), tables->db, tables->table_name);
...
int open_and_lock_tables_derived(THD *thd, TABLE_LIST *tables, bool derived)
{
  ...
  sql_print_information("%s[%d] [tid:%lu]: Try to open and lock tables...", __FILE__, __LINE__, pthread_self());
...
my_bool mysql_rm_tmp_tables(void)
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: Try to cleanup all the directories for temporary tables...", __FILE__, __LINE__, pthread_self());
    ...
    for (idx=0 ; idx < (uint) dirp->number_off_files ; idx++)
    {
                  ...
                  tmp_file_prefix_length))
      {
        ...
        /*
          ...
          files(MYF(0)).
        */
        sql_print_information("%s[%d] [tid: %lu]: Try to delete the file [%] of temporary table...", __FILE__, __LINE__, pthread_self(), filePath);
