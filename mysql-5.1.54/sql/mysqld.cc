...
static void network_init(void)
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: Initializing the network for clients...\n", __FILE__, __LINE__, pthread_self());
...
void close_connection(THD *thd, uint errcode, bool lock)
{
  ...
  sql_print_information("%s[%d] [tid:%lu]: Try to close the connection from client(%s)...", __FILE__, __LINE__, pthread_self(), thd->net.vio->_debug_ip_port);
...
#endif
{
  ...
  */
  ...
  sql_print_information("%s[%d] [tid: %lu]: Start to initialize common variables...\n", __FILE__, __LINE__, pthread_self());
  if (init_common_variables(MYSQL_CONFIG_NAME,
			    argc, argv, load_default_groups))
    unireg_abort(1);				// Will do exit

  sql_print_information("%s[%d] [tid: %lu]: Start to initialize signals...\n", __FILE__, __LINE__, pthread_self());
  init_signals();
  ...
  */
  sql_print_information("%s[%d] [tid: %lu]: Start to check real data directory: %s...\n", __FILE__, __LINE__, pthread_self(), mysql_real_data_home);
...
we force server id to 2, but this MySQL server will not act as a slave.");
  ...
  sql_print_information("%s[%d] [tid: %lu]: Start to initialize server components...\n", __FILE__, __LINE__, pthread_self());
  if (init_server_components())
    unireg_abort(1);

  sql_print_information("%s[%d] [tid: %lu]: Start to initialize SSL...\n", __FILE__, __LINE__, pthread_self());
  init_ssl();

  sql_print_information("%s[%d] [tid: %lu]: Start to initialize network...\n", __FILE__, __LINE__, pthread_self());
  network_init();
...
/**
 * 创建一个线程来处理客户端的请求(one-thread-per-connection模式)
 */
void create_thread_to_handle_connection(THD *thd)
{
  ...
  else
  {
                              ...
                              (void*) thd)))
    {
      ...
      sql_print_information("%s[%d] [tid:%lu]: It's failed to create a new thread to process the requests of client(%s)...", __FILE__, __LINE__, pthread_self(), thd->net.vio->_debug_ip_port);
...
static void create_new_thread(THD *thd)
{
  ...
  */
  ...
  if (connection_count >= max_connections + 1 || abort_loop)
  {
    ...
    sql_print_information("%s[%d] [tid:%lu]: Abort to get/create a thread to process the requests from client(%lu), because of too many connections(%u)...", __FILE__, __LINE__, pthread_self(), thd->net.vio->_debug_ip_port, max_connections);
...
pthread_handler_t handle_connections_sockets(void *arg __attribute__((unused)))
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: Main thread waiting for connections...\n", __FILE__, __LINE__, pthread_self());
