...
void kill_mysql(void)
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: Killing MySQL...", __FILE__, __LINE__, pthread_self());
  ...
  sql_print_information("%s[%d] [tid: %lu]: abort_loop = %d.", __FILE__, __LINE__, pthread_self(), abort_loop);
...
extern "C" sig_handler print_signal_warning(int sig)
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: Got signal %d from thread %ld.", __FILE__, __LINE__, pthread_self(), sig, my_thread_id());
...
static void set_ports()
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: mysqld_port = %d, opt_disable_networking = %d", __FILE__, __LINE__, pthread_self(), mysqld_port, opt_disable_networking);
  ...
  sql_print_information("%s[%d] [tid: %lu]: mysqld_port = %d, opt_disable_networking = %d, mysqld_unix_port = %s", __FILE__, __LINE__, pthread_self(), mysqld_port, opt_disable_networking, mysqld_unix_port);
...
static struct passwd *check_user(const char *user)
{
  ...
  uid_t user_id= geteuid();	//取得执行目前进程有效的用户识别码
  ...
  if (user_id)
  {
    if (user)
    {
      ...
      tmp_user_info= getpwnam(user);	//获取用户的登录相关信息
      //配置的用户与当前进程的启动用户不一致
 ...
 * 函数说明:	根据指定的地址族、数据类型和协议来分配一个套接口的描述字及其所用的资源
 * 返回值:	成功，返回socket文件描述符；失败，返回－1，并设置errno
 * 参数说明:	domain		指明所使用的协议族，通常为PF_INET，表示TCP/IP协议
 * 			type		参数指定socket的类型，基本上有三种：
 * 								数据流套接字(SOCK_STREAM)
 * 								数据报套接字(SOCK_DGRAM)
 * 								原始套接字(SOCK_RAW)
 * 			protocol	通常赋值"0"。
...
static void network_init(void)
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: Initializing the network for clients...", __FILE__, __LINE__, pthread_self());
  ...
  sql_print_information("%s[%d] [tid: %lu]: mysqld_port = %d, opt_disable_networking = %d, opt_bootstrap = %d", __FILE__, __LINE__, pthread_self(), mysqld_port, opt_disable_networking, opt_bootstrap);
  ...
  if (mysqld_port != 0 && !opt_disable_networking && !opt_bootstrap)
  {
    ...
    */
    ...
    {
      sql_print_information("%s[%d] [tid: %lu]: Retrying bind on TCP/IP port %u", __FILE__, __LINE__, pthread_self(), mysqld_port);
    ...
    sql_print_information("%s[%d] [tid: %lu]: Retrying listen on TCP/IP port %u", __FILE__, __LINE__, pthread_self(), mysqld_port);
  ...
  */
  if (mysqld_unix_port[0] && !opt_bootstrap)
  {
    ...
    sql_print_information("%s[%d] [tid: %lu]: Retrying bind on unix socket: %s", __FILE__, __LINE__, pthread_self(), mysqld_unix_port);
    ...
    sql_print_information("%s[%d] [tid: %lu]: Retrying listen on unix socket: %s", __FILE__, __LINE__, pthread_self(), mysqld_unix_port);
...
void close_connection(THD *thd, uint errcode, bool lock)
{
  ...
  sql_print_information("%s[%d] [tid:%lu]: close the connection from client(%lu)...", __FILE__, __LINE__, pthread_self(), thd->net.vio->_debug_ip_port);
...
extern "C" sig_handler end_thread_signal(int sig __attribute__((unused)))
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: Got a signal %d to end thread from thread %ld.", __FILE__, __LINE__, pthread_self(), sig, my_thread_id());
...
bool one_thread_per_connection_end(THD *thd, bool put_in_cache)
{
  ...
  sql_print_information("%s[%d] [tid:%lu]: current thread exit.", __FILE__, __LINE__, pthread_self());
 ...
 *
 * ***************************************************************************************************************************************
 * 函数原型:	int sigprocmask(int how, const sigset_t *restrict set, sigset_t *restrict oldset);
 * 函数说明:	检测或改变目前的信号屏蔽字.其操作依参数how来决定,如果参数oldset不是NULL指针,那么目前的信号屏蔽字会
 * 			由此指针返回.如果set是一个非空指针,则参数how指示如何修改当前信号屏蔽字
 * 参数说明:	参数how:
 * 				IG_BLOCK	该进程新的信号屏蔽字是其当前信号屏蔽字和set指向信号集的并集.set包含了我们希望阻塞的附加信号
 ...
 * 函数说明:	设置信号的处理函数
 * 参数说明:	参数signum可以指定SIGKILL和SIGSTOP以外的所有信号
 *
 * ***************************************************************************************************************************************
 ...
 * 函数说明:	将参数signum代表的信号从参数set信号集里删除
 * 函数返回:	执行成功则返回0，如果有错误则返回-1.
 * 			错误代码:
 * 				EFAULT 参数set指针地址无法存取
...
static void init_signals(void)
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: Initializing signals...", __FILE__, __LINE__, pthread_self());
...
static void start_signal_handler(void)
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: Try to create a thread to handle signals...", __FILE__, __LINE__, pthread_self());
...
pthread_handler_t signal_hand(void *arg __attribute__((unused)))
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: The Signal Handler setup...", __FILE__, __LINE__, pthread_self());
  ...
  */
  ...
  for (;;)
  {
    ...
    sql_print_information("%s[%d] [tid: %lu]: Start to wait for Signal...", __FILE__, __LINE__, pthread_self());
    ...
    sql_print_information("%s[%d] [tid: %lu]: Try to handle the  Signal[%d]...", __FILE__, __LINE__, pthread_self(), sig);
    ...
    case THR_SERVER_ALARM:
      sql_print_information("%s[%d] [tid: %lu]: process alarm by the Signal[%d]...", __FILE__, __LINE__, pthread_self(), sig);
				 ...
				 char **argv, const char **groups)
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: Initializing common variables...\n", __FILE__, __LINE__, pthread_self());
...
static int init_server_components()
{
  ...
  sql_print_information("%s[%d] [tid:%lu]: Initializing server components...", __FILE__, __LINE__, pthread_self());
  ...
  */
  ...
  sql_print_information("%s[%d] [tid:%lu]: query_cache_limit = %d, query_cache_min_res_unit = %d, query_cache_size = %d", __FILE__, __LINE__, pthread_self(), query_cache_limit, query_cache_min_res_unit, query_cache_size);
  ...
  sql_print_information("%s[%d] [tid:%lu]: opt_error_log = %d, opt_help = %d ", __FILE__, __LINE__, pthread_self(), opt_error_log, opt_help);
  ...
  */
  ...
  {
	sql_print_information("%s[%d] [tid:%lu]: Start to setup logs...", __FILE__, __LINE__, pthread_self());
    ...
    sql_print_information("%s[%d] [tid:%lu]: log_error_file = [%s]", __FILE__, __LINE__, pthread_self(), log_error_file);
  }

  sql_print_information("%s[%d] [tid:%lu]: Start to initialize the cache of xid ...", __FILE__, __LINE__, pthread_self());
  if (xid_cache_init())
  {
  ...
  sql_print_information("%s[%d] [tid:%lu]: opt_update_log = %d, opt_bin_log = %d, opt_ignore_builtin_innodb = %d, opt_log_slave_updates = %d", __FILE__, __LINE__, pthread_self(), opt_update_log, opt_bin_log, opt_ignore_builtin_innodb, opt_log_slave_updates);
  ...
  if (opt_update_log)
  {
    ...
    */
	...
	sql_print_information("%s[%d] [tid:%lu]: Start to configure logging before initializing storage engines...", __FILE__, __LINE__, pthread_self());

    if (opt_bin_log)
    {
      opt_sql_bin_update= 0;
      sql_print_error("The update log is no longer supported by MySQL in version 5.0 and above. It is replaced by the binary log.");
    }
    else
    {
...
with --log-bin instead.");
    ...
    sql_print_information("%s[%d] [tid:%lu]: opt_bin_logname = [%s]", __FILE__, __LINE__, pthread_self(), opt_bin_logname);

  }

  sql_print_information("%s[%d] [tid:%lu]: opt_update_log = %d, opt_bin_log = %d, opt_log_slave_updates = %d", __FILE__, __LINE__, pthread_self(), opt_update_log, opt_bin_log, opt_log_slave_updates);
  ...
  else if (opt_binlog_format_id == BINLOG_FORMAT_UNSPEC)
      global_system_variables.binlog_format= BINLOG_FORMAT_STMT;
  else
  {
...
a file name for --log-bin-index option", opt_binlog_index_name);
    ...
    sql_print_information("%s[%d] [tid:%lu]: opt_bin_logname = [%s], opt_binlog_index_name = [%s]", __FILE__, __LINE__, pthread_self(), opt_bin_logname, opt_binlog_index_name);

  }

  /* call ha_init_key_cache() on all key caches to init them */
  sql_print_information("%s[%d] [tid:%lu]: Start to initialize a key cache...", __FILE__, __LINE__, pthread_self());
  ...
  //加载所有的插件
  { 
	sql_print_information("%s[%d] [tid:%lu]: Start to initialize plugins...", __FILE__, __LINE__, pthread_self());
  ...
  if (defaults_argc > 1)
  {
    ...
    */
    ...
    sql_print_information("%s[%d] [tid:%lu]: Start to check if there are any other unknown options...", __FILE__, __LINE__, pthread_self());

    for(int i =0; i<defaults_argc; i++)
    	sql_print_information("%s: ", tmp_argv[i]);
  ...
  if (!errmesg[0][0]){
	sql_print_error("%s[%d] [tid:%lu]: The errmsg.sys is not loaded, terminate to maintain behaviour.", __FILE__, __LINE__, pthread_self());
    unireg_abort(1);
  }
  ...
  */
  ...
  sql_print_information("%s[%d] [tid:%lu]: opt_bin_log = %d ", __FILE__, __LINE__, pthread_self(), opt_bin_log);
...
#endif
{
  printf("%s[%d] [tid: %lu]: Start to initialize my_sys library and pthreads...\n", __FILE__, __LINE__, pthread_self());
  MY_INIT(argv[0]);		// init my_sys library & pthreads
  /* nothing should come before this line ^^^ */
  printf("%s[%d] [tid: %lu]: Initialized my_sys library and pthreads.\n", __FILE__, __LINE__, pthread_self());
...
#endif
  printf("%s[%d] [tid: %lu]: The signal(kill MySQL) = %d.\n", __FILE__, __LINE__, pthread_self(), thr_kill_signal);

  printf("%s[%d] [tid: %lu]: Start to initialize basic logger...\n", __FILE__, __LINE__, pthread_self());
  /*
    Perform basic logger initialization logger. Should be called after
    MY_INIT, as it initializes mutexes. Log tables are inited later.
  */
  logger.init_base();
  sql_print_information("%s[%d] [tid: %lu]: Initialized basic logger.\n", __FILE__, __LINE__, pthread_self());
  ...
  */
  ...
  sql_print_information("%s[%d] [tid: %lu]: Start to initialize common variables...", __FILE__, __LINE__, pthread_self());
  if (init_common_variables(MYSQL_CONFIG_NAME,
			    argc, argv, load_default_groups))
    unireg_abort(1);				// Will do exit

  sql_print_information("%s[%d] [tid: %lu]: Initialized common variables.", __FILE__, __LINE__, pthread_self());


  sql_print_information("%s[%d] [tid: %lu]: Start to initialize signals...", __FILE__, __LINE__, pthread_self());
  init_signals();
  ...
  */
  sql_print_information("%s[%d] [tid: %lu]: Start to check real data directory: [%s]...", __FILE__, __LINE__, pthread_self(), mysql_real_data_home);
  ...
  sql_print_information("%s[%d] [tid: %lu]: Start to check the user: [%s]...", __FILE__, __LINE__, pthread_self(), mysqld_user);
  ...
  sql_print_information("%s[%d] [tid: %lu]: opt_bin_log = %d, server_id = %d ", __FILE__, __LINE__, pthread_self(), opt_bin_log, server_id);
...
we force server id to 2, but this MySQL server will not act as a slave.");
  ...
  sql_print_information("%s[%d] [tid: %lu]: Start to initialize server components...", __FILE__, __LINE__, pthread_self());
  if (init_server_components())
    unireg_abort(1);

  sql_print_information("%s[%d] [tid: %lu]: Initialized server components.", __FILE__, __LINE__, pthread_self());

  sql_print_information("%s[%d] [tid: %lu]: Start to initialize SSL...", __FILE__, __LINE__, pthread_self());
  init_ssl();

  sql_print_information("%s[%d] [tid: %lu]: Start to initialize network...", __FILE__, __LINE__, pthread_self());
  network_init();
  ...
  */
  ...
  sql_print_information("%s[%d] [tid: %lu]: opt_noacl = %d, default_tz_name = %s, opt_bootstrap = %d", __FILE__, __LINE__, pthread_self(), opt_noacl, default_tz_name, opt_bootstrap);
  ...
  */
  ...
  sql_print_information("%s[%d] [tid: %lu]: opt_init_file = %s, opt_bootstrap = %d", __FILE__, __LINE__, pthread_self(), opt_init_file, opt_bootstrap);
  ...
  (void) pthread_mutex_lock(&LOCK_thread_count);
  sql_print_information("%s[%d] [tid: %lu]: Main thread waiting for ready to exit...\n", __FILE__, __LINE__, pthread_self());
  while (!ready_to_exit)
  ...
  sql_print_information("%s[%d] [tid: %lu]: Main thread waiting for signal to end...\n", __FILE__, __LINE__, pthread_self());
  wait_for_signal_thread_to_end();
...
void create_thread_to_handle_connection(THD *thd)
{
  ...
  {
	sql_print_information("%s[%d] [tid:%lu]: Getting a cached thread to process the requests of client(%s)...", __FILE__, __LINE__, pthread_self(), thd->net.vio->_debug_ip_port);
  ...
  {
	sql_print_information("%s[%d] [tid:%lu]: Creating a new thread to process the requests of client(%s)...", __FILE__, __LINE__, pthread_self(), thd->net.vio->_debug_ip_port);
...
static void create_new_thread(THD *thd)
{
  ...
  sql_print_information("%s[%d] [tid:%lu]: Start to get/create a thread to process the requests from client(%lu)...", __FILE__, __LINE__, pthread_self(), thd->net.vio->_debug_ip_port);

  sql_print_information("%s[%d] [tid:%lu]: protocol_version = %d, abort_loop = %d, connection_count = %d, max_connections = %d, thread_count = %d from client(%lu)...", __FILE__, __LINE__, pthread_self(), protocol_version, abort_loop, connection_count, max_connections, thread_count, thd->net.vio->_debug_ip_port);
  ...
  */
  ...
  sql_print_information("%s[%d] [tid:%lu]: connection_count = %d, max_connections = %d, thread_count = %d, thread_id = %d from client(%lu)...", __FILE__, __LINE__, pthread_self(), connection_count, max_connections, thread_count, thread_id, thd->net.vio->_debug_ip_port);
...
pthread_handler_t handle_connections_sockets(void *arg __attribute__((unused)))
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: Main thread waiting for connections...", __FILE__, __LINE__, pthread_self());

  while (!abort_loop)
  {
	sql_print_information("%s[%d] [tid: %lu]: abort_loop = %d, max_used_connection = %d", __FILE__, __LINE__, pthread_self(), abort_loop, max_used_connection);

    readFDs = clientFDs;

    /**
     * 函数原型: int select(int nfds, fd_set* readfds,　fd_set* writefds, fd_set* exceptfds,　const struct timeval* timeout);
     * 功能描述: 用于确定一个或多个套接口的状态,对每一个套接口,调用者可查询它的可读性/可写性及错误状态信息.
     *         用fd_set结构来表示一组等待检查的套接口,在调用返回时,这个结构存有满足一定条件的套接口组的子集.
     *         返回满足条件的套接口的数目
     * 参数说明: nfds      是一个整数值,是指集合中所有文件描述符的范围,即所有文件描述符的最大值加1,不能错！
    ...
    sql_print_information("%s[%d] [tid:%lu]: Accepted a connection from client: %s:%u...", __FILE__, __LINE__, pthread_self(), inet_ntoa(cAddr.sin_addr), ntohs(cAddr.sin_port));
    ...
    */
    ...
    //sql_print_information("%s[%d] [tid:%lu]: Try to get/create a thread to process the requests of client(%s:%u)...", __FILE__, __LINE__, pthread_self(), inet_ntoa(cAddr.sin_addr), ntohs(cAddr.sin_port));
    //strcpy(thd->net.vio->_debug_ip_port, inet_ntoa(cAddr.sin_addr));
    sprintf(thd->net.vio->_debug_ip_port, "%s:%u",inet_ntoa(cAddr.sin_addr), ntohs(cAddr.sin_port));
    sql_print_information("%s[%d] [tid:%lu]: Try to get/create a thread to process the requests of client(%s)...", __FILE__, __LINE__, pthread_self(), thd->net.vio->_debug_ip_port);

    create_new_thread(thd);
  }

  sql_print_information("%s[%d] [tid: %lu]: Main thread exit the waiting for connections...", __FILE__, __LINE__, pthread_self());
...
/**
 * 3种连接管理方式:
 *  1.No-Threads (处理连接使用主线程处理,不额外创建线程)
 *  2.One-Thread-Per-Connection (为每一个连接创建一个线程服务)
 *  3.Pool-Threads (线程池方式)
 * */
  ...
  sql_print_information("%s[%d] [tid: %lu]: global_system_variables.thread_handling = %d.\n", __FILE__, __LINE__, pthread_self(), global_system_variables.thread_handling);
...
static void create_pid_file()
{
  ...
  sql_print_information("%s[%d] [tid: %lu]: Try to create PID file [%s]...", __FILE__, __LINE__, pthread_self(), pidfile_name);
