...
static st_plugin_dl *plugin_dl_add(const LEX_STRING *dl, int report)
{
  ...
  */
  ...
  sql_print_information("%s[%d] [tid: %lu]: Try to open the dll handle {%s}.", __FILE__, __LINE__, pthread_self(), dlpath);
...
static int plugin_initialize(struct st_plugin_int *plugin)
{
  ...
  sql_print_information("%s[%d] [tid:%lu]: Initializing plugin '%s'...", __FILE__, __LINE__, pthread_self(), plugin->name.str);
  ...
  if (plugin_type_initialize[plugin->plugin->type])
  {
    ...
    sql_print_information("%s[%d] [tid:%lu]: Plugin '%s' registration as a %s successfully.", __FILE__, __LINE__, pthread_self(),
    		plugin->name.str, plugin_type_names[plugin->plugin->type].str);
  ...
  else if (plugin->plugin->init)
  {
    ...
    sql_print_information("%s[%d] [tid:%lu]: Plugin '%s' init function returned ok.", __FILE__, __LINE__, pthread_self(),
        		plugin->name.str);
...
int plugin_init(int *argc, char **argv, int flags)
{
  ...
  sql_print_information("%s[%d] [tid:%lu]: Loading and initializing all compiled plugins...", __FILE__, __LINE__, pthread_self());
  ...
  sql_print_information("%s[%d] [tid:%lu]: opt_ignore_builtin_innodb = %d.", __FILE__, __LINE__, pthread_self(), opt_ignore_builtin_innodb);

  sql_print_information("%s[%d] [tid:%lu]: Start to register builtin plugins...", __FILE__, __LINE__, pthread_self());
  ...
  */
  ...
  sql_print_information("%s[%d] [tid:%lu]: Start to register all dynamic plugins...", __FILE__, __LINE__, pthread_self());

  sql_print_information("%s[%d] [tid:%lu]: skip_dynamic_plugins_loading = %d, skip_dynamic_plugins_table_init = %d, opt_plugin_load = {%s}.", __FILE__, __LINE__, pthread_self(), flags & PLUGIN_INIT_SKIP_DYNAMIC_LOADING, flags & PLUGIN_INIT_SKIP_PLUGIN_TABLE, opt_plugin_load);
  ...
  sql_print_information("%s[%d] [tid:%lu]: skip_plugins_initialization = %d.", __FILE__, __LINE__, pthread_self(), flags & PLUGIN_INIT_SKIP_INITIALIZATION);

  if (flags & PLUGIN_INIT_SKIP_INITIALIZATION)
    goto end;

  sql_print_information("%s[%d] [tid:%lu]: Start to initialize all remaining plugins.", __FILE__, __LINE__, pthread_self());
...
/*
 * called only by plugin_init()
 * 解析--plugin-load参数中配置的插件 (例如:--plugin-load=innodb=ha_innodb_plugin.so)
*/
static bool plugin_load_list(MEM_ROOT *tmp_root, int *argc, char **argv,
                             const char *list)
{
  ...
  while (list)
  {
    ...
    case ';':
      ...
      else
      {
        ...
        sql_print_information("%s[%d] [tid: %lu]: Try to add plugin named '%s' with soname '%s'.", __FILE__, __LINE__, pthread_self(), name.str, dl.str);
