			...
			os_thread_create */
{
	...
	fprintf(stderr, "%s[%d] [tid:%lu]: InnoDB Status Monitor Thread startup...\n", __FILE__, __LINE__, pthread_self());
			...
			os_thread_create */
{
	...
	fprintf(stderr, "%s[%d] [tid:%lu]: InnoDB Lock_Timeout Watch Thread startup...\n", __FILE__, __LINE__, pthread_self());
			...
			os_thread_create */
{
	...
	fprintf(stderr, "%s[%d] [tid:%lu]: InnoDB Error Monitor Thread startup...\n", __FILE__, __LINE__, pthread_self());
...
/**
 * InnoDB存储引擎将redo日志缓冲区的内容刷新到redo日志文件
 */
static
void
...
/*===================================*/
{
	...
	fprintf(stderr, "%s[%d] [tid:%lu]: Try to flush redo log buffer to disk{srv_log_writes_and_flush = %lu}...\n", __FILE__, __LINE__, pthread_self(), srv_log_writes_and_flush);
			...
			os_thread_create */
{
	...
	fprintf(stderr, "%s[%d] [tid:%lu]: InnoDB Master Thread startup...\n", __FILE__, __LINE__, pthread_self());
...
loop:  //主循环
	...
	for (i = 0; i < 10; i++) {
		...
		fprintf(stderr, "%s[%d] [tid:%lu][main_1_second_loops]: srv_fast_shutdown = %lu, srv_shutdown_state = %d.\n", __FILE__, __LINE__, pthread_self(), srv_fast_shutdown, srv_shutdown_state);
		...
		srv_sync_log_buffer_in_background(); //保证每秒将redo日志缓冲区的数据刷新到磁盘
		...
		fprintf(stderr, "%s[%d] [tid:%lu][1s_loop]: n_pend_ios = %lu, n_ios = %lu, n_ios_old = %lu.\n", __FILE__, __LINE__, pthread_self(), n_pend_ios, n_ios, n_ios_old);
		...
		if (UNIV_UNLIKELY(buf_get_modified_ratio_pct()
				  > srv_max_buf_pool_modified_pct)) { //脏页在缓存中的比例超过设置的阈值,则刷新100个脏页到磁盘
			...
			skip_sleep = TRUE; //当前io可能繁忙,取消sleep操作立即进行下一轮刷新操作
		} else if (srv_adaptive_flushing) { //自适应地刷新脏页

			/* Try to keep the rate of flushing of dirty
			pages such that redo log generation does not
			produce bursts of IO at checkpoint time. */
			ulint n_flush = buf_flush_get_desired_flush_rate(); //计算当前需要刷新脏页的数量

			if (n_flush) {
				srv_main_thread_op_info =
					"flushing buffer pool pages";
				n_flush = ut_min(PCT_IO(100), n_flush);  //刷新脏页的数量不允许超过100页
				n_pages_flushed =
					buf_flush_batch(
						BUF_FLUSH_LIST,
						n_flush,
				...
				if (n_flush == PCT_IO(100)) { //当前io可能繁忙,取消sleep操作立即进行下一轮刷新操作
					skip_sleep = TRUE;
				}
			}
		}

		if (srv_activity_count == old_activity_count) { //当前没有用户操作
	...
	fprintf(stderr, "%s[%d] [tid:%lu][10s_loop]: n_pend_ios = %lu, n_ios = %lu, n_ios_old = %lu.\n", __FILE__, __LINE__, pthread_self(), n_pend_ios, n_ios, n_ios_old);
...
suspend_thread: //暂停循环
	...
	fprintf(stderr, "%s[%d] [tid:%lu]: Waiting for server activity.\n", __FILE__, __LINE__, pthread_self(), n_pend_ios, n_ios, n_ios_old);
