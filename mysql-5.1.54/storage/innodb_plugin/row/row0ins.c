	...
	que_thr_t*	thr)	/*!< in: query thread */
{
	...
	fprintf(stderr, "%s[%d] [tid: %lu]: Inserting a index(%s) to the table(%s){mode = %u, stored_columns = %u, check_unique_secondary = %u, index_is_unique = %u, index_is_clust = %u}...\n",
			__FILE__, __LINE__, pthread_self(),
			index->name, index->table->name, mode, n_ext,
			thr_get_trx(thr)->check_unique_secondary, dict_index_is_unique(index), dict_index_is_clust(index));
	...
	que_thr_t*	thr)	/*!< in: query thread */
{
	...
	fprintf(stderr, "%s[%d] [tid: %lu]: Inserting a index(%s) to the table(%s){foreign = %d, stored_columns = %u}...\n", __FILE__, __LINE__, pthread_self(), index->name, index->table->name, foreign, n_ext);

	if (foreign && UT_LIST_GET_FIRST(index->table->foreign_list)) { //外键依赖一致性检查
	...
	que_thr_t*	thr)	/*!< in: query thread */
{
	...
	fprintf(stderr, "%s[%d] [tid: %lu]: Try to insert a index(%s) to the table(%s)...\n", __FILE__, __LINE__, pthread_self(), node->index->name, node->table->name);
	...
	que_thr_t*	thr)	/*!< in: query thread */
{
	...
	fprintf(stderr, "%s[%d] [tid: %lu]: node information{state = %u, ins_type = %u}...\n", __FILE__, __LINE__, pthread_self(), node->state, node->ins_type);
