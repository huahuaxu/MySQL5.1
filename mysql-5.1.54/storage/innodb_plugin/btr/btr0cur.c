	...
	mtr_t*		mtr)	/*!< in: mtr */
{
	...
	fprintf(stderr, "%s[%d] [tid: %lu]: Searching the index(%s.%s) tree and positions a tree cursor on the given level(%u)...\n", __FILE__, __LINE__, pthread_self(), index->table->name, index->name, level);
	...
	if (rw_lock_get_writer(&btr_search_latch) == RW_LOCK_NOT_LOCKED
		...
		fprintf(stderr, "%s[%d] [tid: %lu]: Search using the hash index succeeded for the index(%s.%s)...\n", __FILE__, __LINE__, pthread_self(), index->table->name, index->name);
	...
	for (;;) {
		...
		fprintf(stderr, "%s[%d] [tid: %lu]: index_name = (%s.%s), space = %lu, page_no = %lu, insert_planned = %lu, height = %lu, latch_mode = %lu.\n",
				__FILE__, __LINE__, pthread_self(),
				index->table->name, index->name, space, page_no,
				insert_planned, height, latch_mode);
		...
		if (height == 0 && latch_mode <= BTR_MODIFY_LEAF) {
			...
			if (insert_planned
			    && ibuf_should_try(index, ignore_sec_unique)) { //判断当前索引,是否可以使用insert buffer
