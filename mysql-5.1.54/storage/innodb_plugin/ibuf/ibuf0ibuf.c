...
/**
 * 当数据库需要对2级索引(secondary index)进行修改时,先不从外存读页面,而是将这些更新缓存在内存中;
 * 在特定的条件下,统一将这些更新apply到相应的2级索引(secondary index)页面上,这样做可以减少读IO的次数,并且相邻的页面的读IO可以合并.
 * 之所以无法对主键和唯一索引进行优化是因为惟一性判断需要读取实际索引页面中的数据,这就和insert buffer 减少读io的初衷背离了,即Insert对于主键来说是一个顺序IO,
 * 对于secondary index更新操作则是一个随机IO,Insert Buffer可以将多个随机IO merge成一个有利于减少随机IO.
 * 插入缓冲主要带来如下两个坏处:
 *   (1).可能导致数据库宕机后实例恢复时间变长.如果应用程序执行大量的插入和更新操作,且涉及非唯一的聚集索引,一旦出现宕机,
 *       这时就有大量内存中的插入缓冲区数据没有合并至索引页中,导致实例恢复时间会很长.
			...
			to complete */
{
	...
	fprintf(stderr, "%s[%d] [tid:%lu]: Doing insert buffer merge{sync = %u, ibuf.empty = %u}..\n", __FILE__, __LINE__, pthread_self(), sync, ibuf->empty);
			...
			them */
{
	...
	fprintf(stderr, "%s[%d] [tid:%lu]: Try to do insert buffer merge{sync = %u, n_pages = %u}..\n", __FILE__, __LINE__, pthread_self(), sync, n_pages);
	...
	while (sum_pages < n_pages) {
		...
		if (n_bytes == 0) {
			...
			fprintf(stderr, "%s[%d] [tid:%lu]: Done insert buffer merge{sum_bytes = %lu, sum_pages = %lu}..\n", __FILE__, __LINE__, pthread_self(), sum_bytes, sum_pages);
	...
	que_thr_t*	thr)	/*!< in: query thread */
{
	...
	fprintf(stderr, "%s[%d] [tid: %lu]: Try to insert a index entry{index = %s.%s, space = %lu, page_no = %lu} to the insert buffer...\n", __FILE__, __LINE__, pthread_self(),
			index->table->name, index->name, space, page_no);
