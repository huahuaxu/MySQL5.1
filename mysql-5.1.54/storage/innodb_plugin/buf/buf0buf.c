	...
	mtr_t*		mtr)	/*!< in: mini-transaction */
{
	...
	fprintf(stderr, "%s[%d] [tid: %lu]: Try to get access to a database page{mode = %lu, space = %lu, page_no = %lu}...\n", __FILE__, __LINE__, pthread_self(), mode, space, offset);
...
loop:
	...
	if (block == NULL) {
		...
		fprintf(stderr, "%s[%d] [tid: %lu]: The database page{space_id = %lu, page_no = %lu} is %s in the buffer buf_pool...\n",
				__FILE__, __LINE__, pthread_self(),
				space, offset, block == NULL? "not found":"found");
...
loop2:
	if (block == NULL) {
		...
		if (buf_read_page(space, zip_size, offset)) {
			...
			fprintf(stderr, "%s[%d] [tid: %lu]: The database page{space_id = %lu, page_no = %lu} is found in the file...\n",
